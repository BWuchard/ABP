...
################
### change this between scenarios ###
################
year = 2021

###############
antigens = ['DT', 'DTP', 'HepB', 'Hib', 'IPV', 'HPV', 'MMR', 'PCV', 'Rota', 'TCV', 'YF', 'OPV']
antigen_doses = {'DT':3,'DTP':3,'HepB':3,'Hib':3,'HPV':2, 'IPV':4,'MMR':2,'PCV':3,'Rota':2, 'TCV':5, 'YF':1,'OPV':4}
print(antigens)
n_antigens = len(antigens)
print(n_antigens)
vaccines = ['DT', 'Hib', 'DTP-HepB-Hib-IPV', 'DTP-HepB-IPV', 'DTP-Hib', 'DTP-Hib-IPV', 'TCV', 'DTP-IPV', 'DTP', 'DTP-HepB-Hib', 'HepB', 'HPV', 'IPV', 'MMR', 'PCV', 'Rota', 'YF', 'OPV'] 

# Experimental Lists
PAHO33 = ['Brazil', 'Mexico', 'Bahamas', 'Barbados', 'Trinidad', 'Chile', 'Uruguay', 'St. Kitts and Nevis', 'Antigua', 'Venezuela', 'Argentina', 'Panama', 'Suriname', 'Costa Rica', 'Grenada', 'Colombia', 'Saint Lucia', 'Dominica', 'Saint Vincent', 'Cuba', 'Peru', 'Dominican Rep.', 'Ecuador', 'Jamaica', 'Belize', 'El Salvador', 'Guyana', 'Paraguay', 'Guatemala', 'Bolivia', 'Honduras', 'Nicaragua', 'Haiti']
PAHO31 = ['Bahamas', 'Barbados', 'Trinidad', 'Chile', 'Uruguay', 'St. Kitts and Nevis', 'Antigua', 'Venezuela', 'Argentina', 'Panama', 'Suriname', 'Costa Rica', 'Grenada', 'Colombia', 'Saint Lucia', 'Dominica', 'Saint Vincent', 'Cuba', 'Peru', 'Dominican Rep.', 'Ecuador', 'Jamaica', 'Belize', 'El Salvador', 'Guyana', 'Paraguay', 'Guatemala', 'Bolivia', 'Honduras', 'Nicaragua', 'Haiti']
...
country_status_df = pd.read_csv('C:\\Users\\Bailey\\Downloads\\ResearchJunk\\11_2021_Country_Status_(Server)_data.csv')

# Create a regular expression pattern by joining the filter_list with '|'
pattern = '|'.join(antigens)

# CDC Price data
cdc_pr_df = pd.read_csv('C:\\Users\\Bailey\\Downloads\\ResearchJunk\\98_CDC_Pricing_data.csv')
...
# Paho price data
paho_pr_df = pd.read_csv('C:\\Users\\Bailey\\Downloads\\ResearchJunk\\99_PAHO_Pricing_data.csv')
...
# UNICEF Price data
unicef_pr_df = pd.read_csv('C:\\Users\\Bailey\\Downloads\\ResearchJunk\\910_UNICEF_Pricing_data.csv')
...
capacity_df = pd.read_excel('C:\\Users\\Bailey\\Downloads\\ResearchJunk\\Capacity and Availability from Market Insights CONFIDENTIAL.xlsx')
...
average_pr_df = pd.read_csv('C:\\Users\\Bailey\\Downloads\\ResearchJunk\\42_Average_Vaccine_Pricing_Reference_Table_data.csv')

# C Calculating data
c_calculating_df = pd.read_excel('C:\\Users\\Bailey\\Downloads\\ResearchJunk\\CNumbers.xlsx')
...
file_path = 'C:\\Users\\Bailey\\Downloads\\ResearchJunk\\output.xlsx'

######################
### Change this between scenarios ###
selected_list = PAHO33
######################
...
#### CHANGE THIS BETWEEN SCENARIOS ######################
cp_df_sorted = pd.read_excel('PAHO33criteria.xlsx')
#########################################################
...
# Read the Excel file ### CHANGE THIS BETWEEN PAHO SCENARIOS ###
################################################################

###### CHANGE between scenario #######
excel_file_m = 'C:\\Users\\Bailey\\Downloads\\ResearchJunk\\GaviPAHORecreation\\PAHO\\33\\1M_33.xlsx'  # Replace with the actual file path
df = pd.read_excel(excel_file_m, header=None)
...
if selected_list is PAHO33:
    selected_dataframe = paho_pr_df
if selected_list is PAHO31:
    selected_dataframe = paho_pr_df
...
country_param_df_sum = pd.read_excel('PAHO33criteria.xlsx')
country_param_df_sum = country_param_df_sum['Birth Cohort'].sum()
country_status_df_sum = pd.read_csv('births2017.csv')
...
base_file_name, _ = os.path.splitext(os.path.basename(excel_file_m))

# Create a new text file with the same name as the Excel file and ".dat" extension
output_file_name = f"{base_file_name}.dat"

def write_output_to_file(output):
    with open("1M_33.dat", "a") as file:
        file.write(output + '\n')
